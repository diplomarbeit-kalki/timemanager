...
import { fetchAverageWorkinghoursFromActualMonth, fetchWorkingEmployeesCountFromActualMonth } from "@/app/lib/data/datafetching";
...
const EmployeeChart = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      const averageWorkinghoursData = await fetchAverageWorkinghoursFromActualMonth();
      const workingEmployeesCountData = await fetchWorkingEmployeesCountFromActualMonth();

      const formattedChartData = [
        {
          label: "Durchschnittliche Arbeitsstunden",
          color: colors.green[600],
          values: averageWorkinghoursData.map((item: { x: Date; y: number; }) => ({ x: item.x, y: item.y }))
        },
        {
          label: "Arbeitende Mitarbeiter",
          color: colors.red[500],
          values: workingEmployeesCountData.map((item: { x: Date; y: number; }) => ({ x: item.x, y: item.y }))
        }
      ];
